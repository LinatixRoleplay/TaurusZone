...
if(isset($_SESSION['User']))
{
	...
	if($stmt->rowCount() != 1)
		die('Error: No se encontro el usuario en la base de datos');

	$IDUser = $stmt->fetchColumn();

	$st = $con->prepare("SELECT * FROM logros WHERE ID = :usuario");
...
<option value="12" <?php if($mes_perfil == 12) echo 'selected'; ?>>Diciembre</option>
</select>
